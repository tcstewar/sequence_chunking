...
with model:
    ...
    delay = 0.8
    dt = 0.001
    chunknet = nengolib.networks.LinearNetwork(
    ...
    td = temporal.TemporalDecoder(chunknet, seed=1)
    td.decode(model.motor.input, filename='t10', stim_time=5.0, stim_node=chunknet.input, 
              stim_func=stim_func,
              target_func=target_func)
    nengo.Connection(model.vision.output, chunknet.input, transform=[vocab.parse(vocab.keys[env.seq[0]]).v])
